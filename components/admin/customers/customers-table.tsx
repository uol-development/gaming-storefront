...
// Columns that default to descending on first click; name/email default ascending.
const DEFAULT_DESC: ReadonlySet<string> = new Set<SortableColumn>([
  "order_count",
  "total_spent",
  "last_order_at",
  "created_at",
  ...
  return (
    <div className="flex flex-wrap items-center gap-1">
      {shown.map((tag) => (
        <span
          key={tag}
          className="inline-flex items-center rounded border border-border bg-secondary px-1.5 py-0.5 text-xs text-foreground"
        >
          {tag}
        </span>
      ))}
      {extra > 0 ? (
        <span className="inline-flex items-center rounded border border-border bg-secondary px-1.5 py-0.5 text-xs text-muted-foreground">
          +{extra}
        </span>
      ) : null}
    </div>
...
export function CustomersTable({
  ...
  function handleBulkBlock() {
    ...
    void runAction("block the customers", () => setCustomersBlocked(selectedIds, true), true);
  }

  function handleBulkUnblock() {
    if (working || selectedIds.length === 0) return;
    void runAction("unblock the customers", () => setCustomersBlocked(selectedIds, false), true);
  }

  function handleBulkDelete() {
    if (working || selectedIds.length === 0) return;
    if (!window.confirm(`Archive ${selectedIds.length} customer(s)?`)) return;
    void runAction("archive the customers", () => softDeleteCustomers(selectedIds), true);
  }

  function handleDeleteRow(row: AdminCustomerRow) {
    if (working) return;
    if (!window.confirm(`Archive ${row.email}?`)) return;
    void runAction("archive the customer", () => softDeleteCustomers([row.id]), true);
  }
                  ...
                  <td colSpan={9} className="px-4 py-16">
                    <div className="flex flex-col items-center justify-center gap-3 text-center">
                      ...
                      <Link
                        ...
                        className="inline-flex h-9 items-center gap-1.5 rounded-md border border-border bg-card px-3 text-sm font-medium hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                      >
                        New customer
                      </Link>
                    </div>
                ...
                rows.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 align-middle">
                        <StatusBadge blocked={row.is_blocked} />
                      </td>
