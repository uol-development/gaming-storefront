...
import { permanentlyDeleteProducts, restoreProducts } from "@/lib/admin/products-actions";

export interface ProductsTrashTableProps {
...
export function ProductsTrashTable({ rows }: ProductsTrashTableProps) {
  ...
  async function runPurge(ids: string[], rowId: string | null) {
    ...
    const ok = window.confirm(
      count === 1
        ...
        : `Permanently delete ${count} products? This cannot be undone.`,
    );
    if (!ok) return;
    setError(null);
    if (rowId) setPendingId(rowId);
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            Products you move to trash will appear here, where you can restore them or remove them
            for good.
          </p>
        </div>
      </div>
