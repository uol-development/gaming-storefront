...
import { permanentlyDeleteCategories, restoreCategories } from "@/lib/admin/categories-actions";
...
export function CategoriesTrashTable({ rows }: CategoriesTrashTableProps) {
  ...
  async function runRestore(ids: string[], rowId: string | null) {
    ...
    try {
      const res = await restoreCategories(ids);
      if (!res.ok) {
  ...
  async function runPurge(ids: string[], rowId: string | null) {
    ...
    const ok = window.confirm(
      count === 1
        ...
        : `Permanently delete ${count} categories? This cannot be undone.`,
    );
    if (!ok) return;
    setError(null);
    if (rowId) setPendingId(rowId);
    ...
    try {
      const res = await permanentlyDeleteCategories(ids);
      if (!res.ok) {
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            Categories you move to trash will appear here, where you can restore them or remove them
            for good.
          </p>
        </div>
      </div>
