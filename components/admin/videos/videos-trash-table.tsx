...
import { permanentlyDeleteVideos, restoreVideos } from "@/lib/admin/videos-actions";

export interface VideosTrashTableProps {
  rows: AdminVideoRow[];
}
...
export function VideosTrashTable({ rows }: VideosTrashTableProps) {
  ...
  async function runRestore(ids: string[], rowId: string | null) {
    ...
    try {
      const res = await restoreVideos(ids);
      if (!res.ok) {
  ...
  async function runPurge(ids: string[], rowId: string | null) {
    ...
    const ok = window.confirm(
      ids.length === 1
        ...
        : `Permanently delete ${ids.length} videos? This cannot be undone.`,
    );
    if (!ok) return;
    setError(null);
    if (rowId) setPendingId(rowId);
    ...
    try {
      const res = await permanentlyDeleteVideos(ids);
      if (!res.ok) {
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            Videos you remove appear here, where you can restore them or delete them for good.
          </p>
        </div>
      </div>
    );
          ...
          <table className="w-full min-w-[640px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-border text-left text-xs uppercase tracking-wider text-muted-foreground">
                ...
                <th scope="col" className="px-3 py-2.5 font-medium">Video</th>
                <th scope="col" className="px-3 py-2.5 font-medium">Product</th>
                <th scope="col" className="px-3 py-2.5 font-medium">Removed</th>
