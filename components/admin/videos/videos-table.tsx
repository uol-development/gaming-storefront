...
import type { AdminVideoRow } from "@/lib/admin/videos-queries";
import {
  reorderVideos,
  setVideosActive,
  setVideosFeatured,
...
export function VideosTable({
  ...
  const [orderedRows, setOrderedRows] = useState<AdminVideoRow[]>(rows);
  ...
  const persistOrder = useCallback(
    async (nextRows: AdminVideoRow[]) => {
      setErrorMessage(null);
      setReordering(true);
      try {
        const result = await reorderVideos(nextRows.map((r) => r.id));
        if (!result.ok) {
          setErrorMessage(result.error ?? "Failed to reorder videos.");
        ...
        setErrorMessage(err instanceof Error ? err.message : "Failed to reorder videos.");
        setOrderedRows(rows);
      } finally {
        setReordering(false);
      }
  ...
  function handleToggleActive(row: AdminVideoRow) {
    if (working) return;
    void runAction(
      row.is_active ? "disable the video" : "enable the video",
      () => setVideosActive([row.id], !row.is_active),
      false,
    );
  }

  function handleToggleFeatured(row: AdminVideoRow) {
    if (working) return;
    void runAction(
      row.is_featured ? "unfeature the video" : "feature the video",
      () => setVideosFeatured([row.id], !row.is_featured),
      false,
    );
  }

  function handleDeleteRow(row: AdminVideoRow) {
    if (working) return;
    if (!window.confirm(`Move "${row.title || "this video"}" to trash?`)) return;
    void runAction("move the video to trash", () => softDeleteVideos([row.id]), true);
  }
  ...
  const currentProduct = productChoices.some((p) => p.id === productId) ? productId : "";
    ...
    <div className="space-y-4">
      ...
      {!canReorder ? (
        <p className="text-xs text-muted-foreground">
          Sort by <span className="font-medium text-foreground">Position</span> to reorder the
          homepage carousel.
        </p>
      ) : null}
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-border bg-card px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={() =>
                handleBulk("enable the videos", () => setVideosActive(selectedIds, true))
              }
              disabled={working}
            ...
            <button
              type="button"
              onClick={() =>
                handleBulk("disable the videos", () => setVideosActive(selectedIds, false))
              }
              disabled={working}
              className="inline-flex h-8 items-center gap-1.5 rounded-md border border-border bg-card px-2.5 text-xs font-medium hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50"
            >
            ...
            <button
              type="button"
              onClick={() =>
                handleBulk("feature the videos", () => setVideosFeatured(selectedIds, true))
              }
              disabled={working}
              className="inline-flex h-8 items-center gap-1.5 rounded-md border border-border bg-card px-2.5 text-xs font-medium hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50"
            >
            ...
            <button
              type="button"
              onClick={() =>
                handleBulk("unfeature the videos", () => setVideosFeatured(selectedIds, false))
              }
              disabled={working}
              className="inline-flex h-8 items-center gap-1.5 rounded-md border border-border bg-card px-2.5 text-xs font-medium hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50"
            >
            ...
            <button
              ...
              onClick={() =>
                handleBulk(
                  "move the videos to trash",
                  () => softDeleteVideos(selectedIds),
                  `Move ${selectedIds.length} video(s) to trash?`,
                )
              }
              disabled={working}
          ...
          <table className="w-full min-w-[920px] border-collapse text-sm">
            ...
            <tbody>
              {orderedRows.length === 0 ? (
                <tr>
                  <td colSpan={canReorder ? 9 : 8} className="px-4 py-16">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          {hasFilters
                            ...
                            : "Add a YouTube video to feature it on the homepage."}
                        </p>
                      </div>
                      <Link
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 align-middle">
                        <input
                          type="checkbox"
                          aria-label={`Select ${row.title || "video"}`}
                          checked={isSelected}
                        ...
                        <div className="flex items-center gap-2">
                          <div className="min-w-0">
                            ...
                            <p className="truncate text-xs text-muted-foreground">
                              {row.channel_name ? `@${row.channel_name}` : row.video_id}
                            </p>
                          </div>
                          <span
                            className={cn(
                      ...
                      <td className="px-3 py-2.5 align-middle">
                        {row.product_name ? (
                          <span className="text-foreground">{row.product_name}</span>
                        ) : (
                      ...
                      <td className="px-3 py-2.5 align-middle text-center">
                        <button
                          ...
                          aria-label={
                            row.is_featured ? "Unfeature on homepage" : "Feature on homepage"
                          }
                          title="Feature on homepage"
                          className="grid size-8 place-items-center rounded-md text-muted-foreground hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50"
                        >
                          <Star
                            className={cn(
                              "size-4",
                              row.is_featured && "fill-amber-400 text-amber-500",
                            )}
                          />
                        </button>
                      </td>
                      <td className="px-3 py-2.5 align-middle">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button
                            ...
                            disabled={working}
                            aria-label={`Move ${row.title || "video"} to trash`}
                            title="Move to trash"
